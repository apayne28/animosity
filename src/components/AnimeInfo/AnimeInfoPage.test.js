...
const beBop = {
    data: {
        ...
        url: "http://www.animenewsnetwork.com/encyclopedia/anime.php?id=13",
        },
        {
        name: "Wikipedia",
        url: "http://en.wikipedia.org/wiki/Cowboy_Bebop",
        },
        ],
        streaming: [
        {
...
it.only("renders the anime info page",async  ()   => {
    fetch.mockResponseOnce(beBop)
    const data = await getAnime(1)
    console.log(data)
    render(

        <Router>
            <AnimeInfoPage animeId={data.data.mal_id} />
        </Router>
    )
